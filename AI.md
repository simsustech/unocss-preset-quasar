...
Please convert the following SASS code into an array of JavaScript objects suitable for UnoCSS or Tailwind shortcuts, specifically formatted for Quasar's `Shortcut` type.

Pay close attention to the conversion of SASS syntax to UnoCSS/Tailwind syntax, especially within the `[&]` selector and bracketed expressions.

Here are the key rules for conversion:

- **Variables:** Use the variables from
...
- **Transform:** Convert `transform: rotate3d(0, 0, 1, 90deg)` to `[transform:rotate3d(0_0_1_90deg)]`.
- **RTL:** Handle the RTL specific rules correctly, ensuring the `/* rtl:ignore */` comment is preserved within the string.
- **Descendant Selectors:** Use `_` to represent spaces in descendant selectors within the `[&]` block (e.g., `&_>_.child`).
- **Double Escaping:** Ensure `__` in class names within the `[&]` block is double escaped as `\\_\\_`.
- **Theme Integration:** Each shortcut's value should start with a theme fallback: `theme.quasar?.components?.['component-name'] ?? 'unocss/tailwind-styles'`.

Wrap the resulting array of objects within the following structure:
