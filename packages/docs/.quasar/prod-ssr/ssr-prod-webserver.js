...
const clientManifest = JSON.parse(
  readFileSync(join(rootFolder, './quasar.manifest.json'), 'utf-8')
)
...
async function render(ssrContext) {
  ...
  try {
    ...
    ssrContext._meta.endingHeadTags += renderModulesPreload(
      ssrContext.modules,
      { ssrContext }
    )
...
const middlewareParams = {
  ...
  resolve: {
    ...
    root() {
      return join(rootFolder, ...arguments)
    },
